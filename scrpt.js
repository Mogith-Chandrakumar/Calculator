var screen = document.querySelector('.calculator_screen'),
    ...
    for(var i=0; i < numBtns.length; i++){
        numBtns[i].addEventListener('click', function(e) {
            ...
            if(resultDiplayed === false) {
                screen.value += e.target.innerHTML;
            }

            else if(resultDiplayed === true && (lastChar == "+" || lastChar == "-" || lastChar == "×" || lastChar == "÷"))
                {
                    resultDiplayed = false;
                    screen.value += e.target.innerHTML
                }
    ...
    equalBtn.addEventListener('click', function(){
        ...
        var numbers = inputString.split(/\+|\-|\×|\÷/g).map(Number);
    ...
    parcent.addEventListener('click', function(){
        var par = parseFloat(screen.value /  100)
        screen.value = par
    })
